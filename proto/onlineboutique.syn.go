...
func (m CartItemRaw) GetProductId() string {
	...
	if len(m) < 5 {
		panic(fmt.Sprintf("private getter ProductId called on invalid buffer: len(m)=%d, need at least 5 bytes", len(m)))
	}
	...
	if offsetToPrivate >= len(m) || m[offsetToPrivate] != 0x01 {
		...
		panic(fmt.Sprintf("private getter ProductId called on public-only buffer: offsetToPrivate=%d, len(m)=%d, marker=0x%02x (expected 0x01)", offsetToPrivate, len(m), marker))
	}
	// Field 1 (ProductId): variable-length
...
func (m CartItemRaw) GetQuantity() int32 {
	// ASSERT: Private field requires complete buffer
	if len(m) < 5 {
		panic(fmt.Sprintf("private getter Quantity called on invalid buffer: len(m)=%d, need at least 5 bytes", len(m)))
	}
	offsetToPrivate := int(binary.LittleEndian.Uint32(m[1:5]))
	if offsetToPrivate >= len(m) || m[offsetToPrivate] != 0x01 {
		marker := byte(0)
		...
		panic(fmt.Sprintf("private getter Quantity called on public-only buffer: offsetToPrivate=%d, len(m)=%d, marker=0x%02x (expected 0x01)", offsetToPrivate, len(m), marker))
	}
	// Field 2 (Quantity): fixed-length (4 bytes)
...
func (m *CartItemRaw) SetProductId(v string) error {
	...
	if len(*m) < 5 {
		panic(fmt.Sprintf("private setter ProductId called on invalid buffer: len(m)=%d, need at least 5 bytes", len(*m)))
	}
	offsetToPrivate := int(binary.LittleEndian.Uint32((*m)[1:5]))
	if offsetToPrivate >= len(*m) || (*m)[offsetToPrivate] != 0x01 {
		marker := byte(0)
		...
		panic(fmt.Sprintf("private setter ProductId called on public-only buffer: offsetToPrivate=%d, len(m)=%d, marker=0x%02x (expected 0x01)", offsetToPrivate, len(*m), marker))
	}
	// Field 1 (ProductId): variable-length
	if len(*m) < offsetToPrivate+1+4 {
...
func (m *CartItemRaw) SetQuantity(v int32) error {
	// ASSERT: Private field setter requires complete buffer
	if len(*m) < 5 {
		panic(fmt.Sprintf("private setter Quantity called on invalid buffer: len(m)=%d, need at least 5 bytes", len(*m)))
	}
	offsetToPrivate := int(binary.LittleEndian.Uint32((*m)[1:5]))
	if offsetToPrivate >= len(*m) || (*m)[offsetToPrivate] != 0x01 {
		marker := byte(0)
		...
		panic(fmt.Sprintf("private setter Quantity called on public-only buffer: offsetToPrivate=%d, len(m)=%d, marker=0x%02x (expected 0x01)", offsetToPrivate, len(*m), marker))
	}
	// Field 2 (Quantity): fixed-length (4 bytes)
	if len(*m) < offsetToPrivate+5+4 {
...
func (m AddItemRequestRaw) GetUserId() string {
	// ASSERT: Private field requires complete buffer
	if len(m) < 5 {
		panic(fmt.Sprintf("private getter UserId called on invalid buffer: len(m)=%d, need at least 5 bytes", len(m)))
	}
	offsetToPrivate := int(binary.LittleEndian.Uint32(m[1:5]))
	if offsetToPrivate >= len(m) || m[offsetToPrivate] != 0x01 {
		marker := byte(0)
		...
		panic(fmt.Sprintf("private getter UserId called on public-only buffer: offsetToPrivate=%d, len(m)=%d, marker=0x%02x (expected 0x01)", offsetToPrivate, len(m), marker))
	}
	// Field 1 (UserId): variable-length
	if len(m) < offsetToPrivate+1+4 {
		return ""
...
func (m AddItemRequestRaw) GetItem() CartItemRaw {
	// ASSERT: Private field requires complete buffer
	if len(m) < 5 {
		panic(fmt.Sprintf("private getter Item called on invalid buffer: len(m)=%d, need at least 5 bytes", len(m)))
	}
	offsetToPrivate := int(binary.LittleEndian.Uint32(m[1:5]))
	if offsetToPrivate >= len(m) || m[offsetToPrivate] != 0x01 {
		marker := byte(0)
		...
		panic(fmt.Sprintf("private getter Item called on public-only buffer: offsetToPrivate=%d, len(m)=%d, marker=0x%02x (expected 0x01)", offsetToPrivate, len(m), marker))
	}
	// Field 2 (Item): nested message
	if len(m) < offsetToPrivate+5+4 {
		return nil
...
func (m *AddItemRequestRaw) SetUserId(v string) error {
	// ASSERT: Private field setter requires complete buffer
	if len(*m) < 5 {
		panic(fmt.Sprintf("private setter UserId called on invalid buffer: len(m)=%d, need at least 5 bytes", len(*m)))
	}
	offsetToPrivate := int(binary.LittleEndian.Uint32((*m)[1:5]))
	if offsetToPrivate >= len(*m) || (*m)[offsetToPrivate] != 0x01 {
		marker := byte(0)
		...
		panic(fmt.Sprintf("private setter UserId called on public-only buffer: offsetToPrivate=%d, len(m)=%d, marker=0x%02x (expected 0x01)", offsetToPrivate, len(*m), marker))
	}
	// Field 1 (UserId): variable-length
	if len(*m) < offsetToPrivate+1+4 {
		return fmt.Errorf("buffer too short for table entry")
...
func (m *AddItemRequestRaw) SetItem(v CartItemRaw) error {
	// ASSERT: Private field setter requires complete buffer
	if len(*m) < 5 {
		panic(fmt.Sprintf("private setter Item called on invalid buffer: len(m)=%d, need at least 5 bytes", len(*m)))
	}
	offsetToPrivate := int(binary.LittleEndian.Uint32((*m)[1:5]))
	if offsetToPrivate >= len(*m) || (*m)[offsetToPrivate] != 0x01 {
		marker := byte(0)
		...
		panic(fmt.Sprintf("private setter Item called on public-only buffer: offsetToPrivate=%d, len(m)=%d, marker=0x%02x (expected 0x01)", offsetToPrivate, len(*m), marker))
	}
	// Field 2 (Item): nested message
	if len(*m) < offsetToPrivate+5+4 {
		return fmt.Errorf("buffer too short for table entry")
...
func (m CartRaw) GetItems() []CartItemRaw {
	// ASSERT: Private field requires complete buffer
	if len(m) < 5 {
		panic(fmt.Sprintf("private getter Items called on invalid buffer: len(m)=%d, need at least 5 bytes", len(m)))
	}
	offsetToPrivate := int(binary.LittleEndian.Uint32(m[1:5]))
	if offsetToPrivate >= len(m) || m[offsetToPrivate] != 0x01 {
		marker := byte(0)
		...
		panic(fmt.Sprintf("private getter Items called on public-only buffer: offsetToPrivate=%d, len(m)=%d, marker=0x%02x (expected 0x01)", offsetToPrivate, len(m), marker))
	}
	// Field 2 (Items): repeated nested message
	if len(m) < offsetToPrivate+5+4 {
		return nil
...
func (m *CartRaw) SetItems(v []CartItemRaw) error {
	// ASSERT: Private field setter requires complete buffer
	if len(*m) < 5 {
		panic(fmt.Sprintf("private setter Items called on invalid buffer: len(m)=%d, need at least 5 bytes", len(*m)))
	}
	offsetToPrivate := int(binary.LittleEndian.Uint32((*m)[1:5]))
	if offsetToPrivate >= len(*m) || (*m)[offsetToPrivate] != 0x01 {
		marker := byte(0)
		...
		panic(fmt.Sprintf("private setter Items called on public-only buffer: offsetToPrivate=%d, len(m)=%d, marker=0x%02x (expected 0x01)", offsetToPrivate, len(*m), marker))
	}
	// Field 2 (Items): repeated nested message
	if len(*m) < offsetToPrivate+5+4 {
		return fmt.Errorf("buffer too short for table entry")
...
func (m ListRecommendationsRequestRaw) GetProductIds() []string {
	// ASSERT: Private field requires complete buffer
	if len(m) < 5 {
		panic(fmt.Sprintf("private getter ProductIds called on invalid buffer: len(m)=%d, need at least 5 bytes", len(m)))
	}
	offsetToPrivate := int(binary.LittleEndian.Uint32(m[1:5]))
	if offsetToPrivate >= len(m) || m[offsetToPrivate] != 0x01 {
		marker := byte(0)
		...
		panic(fmt.Sprintf("private getter ProductIds called on public-only buffer: offsetToPrivate=%d, len(m)=%d, marker=0x%02x (expected 0x01)", offsetToPrivate, len(m), marker))
	}
	// Field 2 (ProductIds): repeated variable-length
	if len(m) < offsetToPrivate+5+4 {
		return nil
...
func (m *ListRecommendationsRequestRaw) SetProductIds(v []string) error {
	// ASSERT: Private field setter requires complete buffer
	if len(*m) < 5 {
		panic(fmt.Sprintf("private setter ProductIds called on invalid buffer: len(m)=%d, need at least 5 bytes", len(*m)))
	}
	offsetToPrivate := int(binary.LittleEndian.Uint32((*m)[1:5]))
	if offsetToPrivate >= len(*m) || (*m)[offsetToPrivate] != 0x01 {
		marker := byte(0)
		...
		panic(fmt.Sprintf("private setter ProductIds called on public-only buffer: offsetToPrivate=%d, len(m)=%d, marker=0x%02x (expected 0x01)", offsetToPrivate, len(*m), marker))
	}
	// Field 2 (ProductIds): repeated variable-length
	if len(*m) < offsetToPrivate+5+4 {
		return fmt.Errorf("buffer too short for table entry")
...
func (m ProductRaw) GetPicture() string {
	// ASSERT: Private field requires complete buffer
	if len(m) < 5 {
		panic(fmt.Sprintf("private getter Picture called on invalid buffer: len(m)=%d, need at least 5 bytes", len(m)))
	}
	offsetToPrivate := int(binary.LittleEndian.Uint32(m[1:5]))
	if offsetToPrivate >= len(m) || m[offsetToPrivate] != 0x01 {
		marker := byte(0)
		...
		panic(fmt.Sprintf("private getter Picture called on public-only buffer: offsetToPrivate=%d, len(m)=%d, marker=0x%02x (expected 0x01)", offsetToPrivate, len(m), marker))
	}
	// Field 4 (Picture): variable-length
...
func (m ProductRaw) GetPriceUsd() MoneyRaw {
	// ASSERT: Private field requires complete buffer
	if len(m) < 5 {
		panic(fmt.Sprintf("private getter PriceUsd called on invalid buffer: len(m)=%d, need at least 5 bytes", len(m)))
	}
	offsetToPrivate := int(binary.LittleEndian.Uint32(m[1:5]))
	if offsetToPrivate >= len(m) || m[offsetToPrivate] != 0x01 {
		marker := byte(0)
		...
		panic(fmt.Sprintf("private getter PriceUsd called on public-only buffer: offsetToPrivate=%d, len(m)=%d, marker=0x%02x (expected 0x01)", offsetToPrivate, len(m), marker))
	}
	// Field 5 (PriceUsd): nested message
...
func (m ProductRaw) GetCategories() []string {
	// ASSERT: Private field requires complete buffer
	if len(m) < 5 {
		panic(fmt.Sprintf("private getter Categories called on invalid buffer: len(m)=%d, need at least 5 bytes", len(m)))
	}
	offsetToPrivate := int(binary.LittleEndian.Uint32(m[1:5]))
	if offsetToPrivate >= len(m) || m[offsetToPrivate] != 0x01 {
		marker := byte(0)
		...
		panic(fmt.Sprintf("private getter Categories called on public-only buffer: offsetToPrivate=%d, len(m)=%d, marker=0x%02x (expected 0x01)", offsetToPrivate, len(m), marker))
	}
	// Field 6 (Categories): repeated variable-length
...
func (m *ProductRaw) SetPicture(v string) error {
	// ASSERT: Private field setter requires complete buffer
	if len(*m) < 5 {
		panic(fmt.Sprintf("private setter Picture called on invalid buffer: len(m)=%d, need at least 5 bytes", len(*m)))
	}
	offsetToPrivate := int(binary.LittleEndian.Uint32((*m)[1:5]))
	if offsetToPrivate >= len(*m) || (*m)[offsetToPrivate] != 0x01 {
		marker := byte(0)
		...
		panic(fmt.Sprintf("private setter Picture called on public-only buffer: offsetToPrivate=%d, len(m)=%d, marker=0x%02x (expected 0x01)", offsetToPrivate, len(*m), marker))
	}
	// Field 4 (Picture): variable-length
	if len(*m) < offsetToPrivate+13+4 {
		return fmt.Errorf("buffer too short for table entry")
...
func (m *ProductRaw) SetPriceUsd(v MoneyRaw) error {
	// ASSERT: Private field setter requires complete buffer
	if len(*m) < 5 {
		panic(fmt.Sprintf("private setter PriceUsd called on invalid buffer: len(m)=%d, need at least 5 bytes", len(*m)))
	}
	offsetToPrivate := int(binary.LittleEndian.Uint32((*m)[1:5]))
	if offsetToPrivate >= len(*m) || (*m)[offsetToPrivate] != 0x01 {
		marker := byte(0)
		...
		panic(fmt.Sprintf("private setter PriceUsd called on public-only buffer: offsetToPrivate=%d, len(m)=%d, marker=0x%02x (expected 0x01)", offsetToPrivate, len(*m), marker))
	}
	// Field 5 (PriceUsd): nested message
	if len(*m) < offsetToPrivate+17+4 {
		return fmt.Errorf("buffer too short for table entry")
...
func (m *ProductRaw) SetCategories(v []string) error {
	// ASSERT: Private field setter requires complete buffer
	if len(*m) < 5 {
		panic(fmt.Sprintf("private setter Categories called on invalid buffer: len(m)=%d, need at least 5 bytes", len(*m)))
	}
	offsetToPrivate := int(binary.LittleEndian.Uint32((*m)[1:5]))
	if offsetToPrivate >= len(*m) || (*m)[offsetToPrivate] != 0x01 {
		marker := byte(0)
		...
		panic(fmt.Sprintf("private setter Categories called on public-only buffer: offsetToPrivate=%d, len(m)=%d, marker=0x%02x (expected 0x01)", offsetToPrivate, len(*m), marker))
	}
	// Field 6 (Categories): repeated variable-length
	if len(*m) < offsetToPrivate+21+4 {
		return fmt.Errorf("buffer too short for table entry")
...
func (m ListProductsResponseRaw) GetProducts() []ProductRaw {
	// ASSERT: Private field requires complete buffer
	if len(m) < 5 {
		panic(fmt.Sprintf("private getter Products called on invalid buffer: len(m)=%d, need at least 5 bytes", len(m)))
	}
	offsetToPrivate := int(binary.LittleEndian.Uint32(m[1:5]))
	if offsetToPrivate >= len(m) || m[offsetToPrivate] != 0x01 {
		marker := byte(0)
		...
		panic(fmt.Sprintf("private getter Products called on public-only buffer: offsetToPrivate=%d, len(m)=%d, marker=0x%02x (expected 0x01)", offsetToPrivate, len(m), marker))
	}
	// Field 1 (Products): repeated nested message
	if len(m) < offsetToPrivate+1+4 {
		return nil
...
func (m *ListProductsResponseRaw) SetProducts(v []ProductRaw) error {
	// ASSERT: Private field setter requires complete buffer
	if len(*m) < 5 {
		panic(fmt.Sprintf("private setter Products called on invalid buffer: len(m)=%d, need at least 5 bytes", len(*m)))
	}
	offsetToPrivate := int(binary.LittleEndian.Uint32((*m)[1:5]))
	if offsetToPrivate >= len(*m) || (*m)[offsetToPrivate] != 0x01 {
		marker := byte(0)
		...
		panic(fmt.Sprintf("private setter Products called on public-only buffer: offsetToPrivate=%d, len(m)=%d, marker=0x%02x (expected 0x01)", offsetToPrivate, len(*m), marker))
	}
	// Field 1 (Products): repeated nested message
	if len(*m) < offsetToPrivate+1+4 {
		return fmt.Errorf("buffer too short for table entry")
...
func (m SearchProductsRequestRaw) GetQuery() string {
	// ASSERT: Private field requires complete buffer
	if len(m) < 5 {
		panic(fmt.Sprintf("private getter Query called on invalid buffer: len(m)=%d, need at least 5 bytes", len(m)))
	}
	offsetToPrivate := int(binary.LittleEndian.Uint32(m[1:5]))
	if offsetToPrivate >= len(m) || m[offsetToPrivate] != 0x01 {
		marker := byte(0)
		...
		panic(fmt.Sprintf("private getter Query called on public-only buffer: offsetToPrivate=%d, len(m)=%d, marker=0x%02x (expected 0x01)", offsetToPrivate, len(m), marker))
	}
	// Field 1 (Query): variable-length
	if len(m) < offsetToPrivate+1+4 {
		return ""
...
func (m *SearchProductsRequestRaw) SetQuery(v string) error {
	// ASSERT: Private field setter requires complete buffer
	if len(*m) < 5 {
		panic(fmt.Sprintf("private setter Query called on invalid buffer: len(m)=%d, need at least 5 bytes", len(*m)))
	}
	offsetToPrivate := int(binary.LittleEndian.Uint32((*m)[1:5]))
	if offsetToPrivate >= len(*m) || (*m)[offsetToPrivate] != 0x01 {
		marker := byte(0)
		...
		panic(fmt.Sprintf("private setter Query called on public-only buffer: offsetToPrivate=%d, len(m)=%d, marker=0x%02x (expected 0x01)", offsetToPrivate, len(*m), marker))
	}
	// Field 1 (Query): variable-length
	if len(*m) < offsetToPrivate+1+4 {
		return fmt.Errorf("buffer too short for table entry")
...
func (m SearchProductsResponseRaw) GetResults() []ProductRaw {
	// ASSERT: Private field requires complete buffer
	if len(m) < 5 {
		panic(fmt.Sprintf("private getter Results called on invalid buffer: len(m)=%d, need at least 5 bytes", len(m)))
	}
	offsetToPrivate := int(binary.LittleEndian.Uint32(m[1:5]))
	if offsetToPrivate >= len(m) || m[offsetToPrivate] != 0x01 {
		marker := byte(0)
		...
		panic(fmt.Sprintf("private getter Results called on public-only buffer: offsetToPrivate=%d, len(m)=%d, marker=0x%02x (expected 0x01)", offsetToPrivate, len(m), marker))
	}
	// Field 1 (Results): repeated nested message
	if len(m) < offsetToPrivate+1+4 {
		return nil
...
func (m *SearchProductsResponseRaw) SetResults(v []ProductRaw) error {
	// ASSERT: Private field setter requires complete buffer
	if len(*m) < 5 {
		panic(fmt.Sprintf("private setter Results called on invalid buffer: len(m)=%d, need at least 5 bytes", len(*m)))
	}
	offsetToPrivate := int(binary.LittleEndian.Uint32((*m)[1:5]))
	if offsetToPrivate >= len(*m) || (*m)[offsetToPrivate] != 0x01 {
		marker := byte(0)
		...
		panic(fmt.Sprintf("private setter Results called on public-only buffer: offsetToPrivate=%d, len(m)=%d, marker=0x%02x (expected 0x01)", offsetToPrivate, len(*m), marker))
	}
	// Field 1 (Results): repeated nested message
	if len(*m) < offsetToPrivate+1+4 {
		return fmt.Errorf("buffer too short for table entry")
...
func (m GetQuoteResponseRaw) GetCostUsd() MoneyRaw {
	// ASSERT: Private field requires complete buffer
	if len(m) < 5 {
		panic(fmt.Sprintf("private getter CostUsd called on invalid buffer: len(m)=%d, need at least 5 bytes", len(m)))
	}
	offsetToPrivate := int(binary.LittleEndian.Uint32(m[1:5]))
	if offsetToPrivate >= len(m) || m[offsetToPrivate] != 0x01 {
		marker := byte(0)
		...
		panic(fmt.Sprintf("private getter CostUsd called on public-only buffer: offsetToPrivate=%d, len(m)=%d, marker=0x%02x (expected 0x01)", offsetToPrivate, len(m), marker))
	}
	// Field 1 (CostUsd): nested message
	if len(m) < offsetToPrivate+1+4 {
		return nil
...
func (m *GetQuoteResponseRaw) SetCostUsd(v MoneyRaw) error {
	// ASSERT: Private field setter requires complete buffer
	if len(*m) < 5 {
		panic(fmt.Sprintf("private setter CostUsd called on invalid buffer: len(m)=%d, need at least 5 bytes", len(*m)))
	}
	offsetToPrivate := int(binary.LittleEndian.Uint32((*m)[1:5]))
	if offsetToPrivate >= len(*m) || (*m)[offsetToPrivate] != 0x01 {
		marker := byte(0)
		...
		panic(fmt.Sprintf("private setter CostUsd called on public-only buffer: offsetToPrivate=%d, len(m)=%d, marker=0x%02x (expected 0x01)", offsetToPrivate, len(*m), marker))
	}
	// Field 1 (CostUsd): nested message
	if len(*m) < offsetToPrivate+1+4 {
		return fmt.Errorf("buffer too short for table entry")
...
func (m ShipOrderResponseRaw) GetTrackingId() string {
	// ASSERT: Private field requires complete buffer
	if len(m) < 5 {
		panic(fmt.Sprintf("private getter TrackingId called on invalid buffer: len(m)=%d, need at least 5 bytes", len(m)))
	}
	offsetToPrivate := int(binary.LittleEndian.Uint32(m[1:5]))
	if offsetToPrivate >= len(m) || m[offsetToPrivate] != 0x01 {
		marker := byte(0)
		...
		panic(fmt.Sprintf("private getter TrackingId called on public-only buffer: offsetToPrivate=%d, len(m)=%d, marker=0x%02x (expected 0x01)", offsetToPrivate, len(m), marker))
	}
	// Field 1 (TrackingId): variable-length
	if len(m) < offsetToPrivate+1+4 {
		return ""
...
func (m *ShipOrderResponseRaw) SetTrackingId(v string) error {
	// ASSERT: Private field setter requires complete buffer
	if len(*m) < 5 {
		panic(fmt.Sprintf("private setter TrackingId called on invalid buffer: len(m)=%d, need at least 5 bytes", len(*m)))
	}
	offsetToPrivate := int(binary.LittleEndian.Uint32((*m)[1:5]))
	if offsetToPrivate >= len(*m) || (*m)[offsetToPrivate] != 0x01 {
		marker := byte(0)
		...
		panic(fmt.Sprintf("private setter TrackingId called on public-only buffer: offsetToPrivate=%d, len(m)=%d, marker=0x%02x (expected 0x01)", offsetToPrivate, len(*m), marker))
	}
	// Field 1 (TrackingId): variable-length
	if len(*m) < offsetToPrivate+1+4 {
		return fmt.Errorf("buffer too short for table entry")
...
func (m AddressRaw) GetStreetAddress() string {
	// ASSERT: Private field requires complete buffer
	if len(m) < 5 {
		panic(fmt.Sprintf("private getter StreetAddress called on invalid buffer: len(m)=%d, need at least 5 bytes", len(m)))
	}
	offsetToPrivate := int(binary.LittleEndian.Uint32(m[1:5]))
	if offsetToPrivate >= len(m) || m[offsetToPrivate] != 0x01 {
		marker := byte(0)
		...
		panic(fmt.Sprintf("private getter StreetAddress called on public-only buffer: offsetToPrivate=%d, len(m)=%d, marker=0x%02x (expected 0x01)", offsetToPrivate, len(m), marker))
	}
	// Field 1 (StreetAddress): variable-length
	if len(m) < offsetToPrivate+1+4 {
		return ""
...
func (m AddressRaw) GetZipCode() int32 {
	// ASSERT: Private field requires complete buffer
	if len(m) < 5 {
		panic(fmt.Sprintf("private getter ZipCode called on invalid buffer: len(m)=%d, need at least 5 bytes", len(m)))
	}
	offsetToPrivate := int(binary.LittleEndian.Uint32(m[1:5]))
	if offsetToPrivate >= len(m) || m[offsetToPrivate] != 0x01 {
		marker := byte(0)
		...
		panic(fmt.Sprintf("private getter ZipCode called on public-only buffer: offsetToPrivate=%d, len(m)=%d, marker=0x%02x (expected 0x01)", offsetToPrivate, len(m), marker))
	}
	// Field 5 (ZipCode): fixed-length (4 bytes)
	if len(m) < offsetToPrivate+17+4 {
		return 0
...
func (m *AddressRaw) SetStreetAddress(v string) error {
	// ASSERT: Private field setter requires complete buffer
	if len(*m) < 5 {
		panic(fmt.Sprintf("private setter StreetAddress called on invalid buffer: len(m)=%d, need at least 5 bytes", len(*m)))
	}
	offsetToPrivate := int(binary.LittleEndian.Uint32((*m)[1:5]))
	if offsetToPrivate >= len(*m) || (*m)[offsetToPrivate] != 0x01 {
		marker := byte(0)
		...
		panic(fmt.Sprintf("private setter StreetAddress called on public-only buffer: offsetToPrivate=%d, len(m)=%d, marker=0x%02x (expected 0x01)", offsetToPrivate, len(*m), marker))
	}
	// Field 1 (StreetAddress): variable-length
	if len(*m) < offsetToPrivate+1+4 {
		return fmt.Errorf("buffer too short for table entry")
...
func (m *AddressRaw) SetZipCode(v int32) error {
	// ASSERT: Private field setter requires complete buffer
	if len(*m) < 5 {
		panic(fmt.Sprintf("private setter ZipCode called on invalid buffer: len(m)=%d, need at least 5 bytes", len(*m)))
	}
	offsetToPrivate := int(binary.LittleEndian.Uint32((*m)[1:5]))
	if offsetToPrivate >= len(*m) || (*m)[offsetToPrivate] != 0x01 {
		marker := byte(0)
		...
		panic(fmt.Sprintf("private setter ZipCode called on public-only buffer: offsetToPrivate=%d, len(m)=%d, marker=0x%02x (expected 0x01)", offsetToPrivate, len(*m), marker))
	}
	// Field 5 (ZipCode): fixed-length (4 bytes)
	if len(*m) < offsetToPrivate+17+4 {
		return fmt.Errorf("buffer too short")
...
func (m MoneyRaw) GetCurrencyCode() string {
	// ASSERT: Private field requires complete buffer
	if len(m) < 5 {
		panic(fmt.Sprintf("private getter CurrencyCode called on invalid buffer: len(m)=%d, need at least 5 bytes", len(m)))
	}
	offsetToPrivate := int(binary.LittleEndian.Uint32(m[1:5]))
	if offsetToPrivate >= len(m) || m[offsetToPrivate] != 0x01 {
		marker := byte(0)
		...
		panic(fmt.Sprintf("private getter CurrencyCode called on public-only buffer: offsetToPrivate=%d, len(m)=%d, marker=0x%02x (expected 0x01)", offsetToPrivate, len(m), marker))
	}
	// Field 1 (CurrencyCode): variable-length
	if len(m) < offsetToPrivate+1+4 {
		return ""
...
func (m MoneyRaw) GetUnits() int64 {
	// ASSERT: Private field requires complete buffer
	if len(m) < 5 {
		panic(fmt.Sprintf("private getter Units called on invalid buffer: len(m)=%d, need at least 5 bytes", len(m)))
	}
	offsetToPrivate := int(binary.LittleEndian.Uint32(m[1:5]))
	if offsetToPrivate >= len(m) || m[offsetToPrivate] != 0x01 {
		marker := byte(0)
		...
		panic(fmt.Sprintf("private getter Units called on public-only buffer: offsetToPrivate=%d, len(m)=%d, marker=0x%02x (expected 0x01)", offsetToPrivate, len(m), marker))
	}
	// Field 2 (Units): fixed-length (8 bytes)
...
func (m MoneyRaw) GetNanos() int32 {
	// ASSERT: Private field requires complete buffer
	if len(m) < 5 {
		panic(fmt.Sprintf("private getter Nanos called on invalid buffer: len(m)=%d, need at least 5 bytes", len(m)))
	}
	offsetToPrivate := int(binary.LittleEndian.Uint32(m[1:5]))
	if offsetToPrivate >= len(m) || m[offsetToPrivate] != 0x01 {
		marker := byte(0)
		...
		panic(fmt.Sprintf("private getter Nanos called on public-only buffer: offsetToPrivate=%d, len(m)=%d, marker=0x%02x (expected 0x01)", offsetToPrivate, len(m), marker))
	}
	// Field 3 (Nanos): fixed-length (4 bytes)
	if len(m) < offsetToPrivate+13+4 {
		return 0
...
func (m *MoneyRaw) SetCurrencyCode(v string) error {
	// ASSERT: Private field setter requires complete buffer
	if len(*m) < 5 {
		panic(fmt.Sprintf("private setter CurrencyCode called on invalid buffer: len(m)=%d, need at least 5 bytes", len(*m)))
	}
	offsetToPrivate := int(binary.LittleEndian.Uint32((*m)[1:5]))
	if offsetToPrivate >= len(*m) || (*m)[offsetToPrivate] != 0x01 {
		marker := byte(0)
		...
		panic(fmt.Sprintf("private setter CurrencyCode called on public-only buffer: offsetToPrivate=%d, len(m)=%d, marker=0x%02x (expected 0x01)", offsetToPrivate, len(*m), marker))
	}
	// Field 1 (CurrencyCode): variable-length
	if len(*m) < offsetToPrivate+1+4 {
		return fmt.Errorf("buffer too short for table entry")
...
func (m *MoneyRaw) SetUnits(v int64) error {
	// ASSERT: Private field setter requires complete buffer
	if len(*m) < 5 {
		panic(fmt.Sprintf("private setter Units called on invalid buffer: len(m)=%d, need at least 5 bytes", len(*m)))
	}
	offsetToPrivate := int(binary.LittleEndian.Uint32((*m)[1:5]))
	if offsetToPrivate >= len(*m) || (*m)[offsetToPrivate] != 0x01 {
		marker := byte(0)
		...
		panic(fmt.Sprintf("private setter Units called on public-only buffer: offsetToPrivate=%d, len(m)=%d, marker=0x%02x (expected 0x01)", offsetToPrivate, len(*m), marker))
	}
	// Field 2 (Units): fixed-length (8 bytes)
	if len(*m) < offsetToPrivate+5+8 {
		return fmt.Errorf("buffer too short")
...
func (m *MoneyRaw) SetNanos(v int32) error {
	// ASSERT: Private field setter requires complete buffer
	if len(*m) < 5 {
		panic(fmt.Sprintf("private setter Nanos called on invalid buffer: len(m)=%d, need at least 5 bytes", len(*m)))
	}
	offsetToPrivate := int(binary.LittleEndian.Uint32((*m)[1:5]))
	if offsetToPrivate >= len(*m) || (*m)[offsetToPrivate] != 0x01 {
		marker := byte(0)
		...
		panic(fmt.Sprintf("private setter Nanos called on public-only buffer: offsetToPrivate=%d, len(m)=%d, marker=0x%02x (expected 0x01)", offsetToPrivate, len(*m), marker))
	}
	// Field 3 (Nanos): fixed-length (4 bytes)
	if len(*m) < offsetToPrivate+13+4 {
		return fmt.Errorf("buffer too short")
...
func (m GetSupportedCurrenciesResponseRaw) GetCurrencyCodes() []string {
	// ASSERT: Private field requires complete buffer
	if len(m) < 5 {
		panic(fmt.Sprintf("private getter CurrencyCodes called on invalid buffer: len(m)=%d, need at least 5 bytes", len(m)))
	}
	offsetToPrivate := int(binary.LittleEndian.Uint32(m[1:5]))
	if offsetToPrivate >= len(m) || m[offsetToPrivate] != 0x01 {
		marker := byte(0)
		...
		panic(fmt.Sprintf("private getter CurrencyCodes called on public-only buffer: offsetToPrivate=%d, len(m)=%d, marker=0x%02x (expected 0x01)", offsetToPrivate, len(m), marker))
	}
	// Field 1 (CurrencyCodes): repeated variable-length
	if len(m) < offsetToPrivate+1+4 {
		return nil
...
func (m *GetSupportedCurrenciesResponseRaw) SetCurrencyCodes(v []string) error {
	// ASSERT: Private field setter requires complete buffer
	if len(*m) < 5 {
		panic(fmt.Sprintf("private setter CurrencyCodes called on invalid buffer: len(m)=%d, need at least 5 bytes", len(*m)))
	}
	offsetToPrivate := int(binary.LittleEndian.Uint32((*m)[1:5]))
	if offsetToPrivate >= len(*m) || (*m)[offsetToPrivate] != 0x01 {
		marker := byte(0)
		...
		panic(fmt.Sprintf("private setter CurrencyCodes called on public-only buffer: offsetToPrivate=%d, len(m)=%d, marker=0x%02x (expected 0x01)", offsetToPrivate, len(*m), marker))
	}
	// Field 1 (CurrencyCodes): repeated variable-length
	if len(*m) < offsetToPrivate+1+4 {
		return fmt.Errorf("buffer too short for table entry")
...
func (m CurrencyConversionRequestRaw) GetFrom() MoneyRaw {
	// ASSERT: Private field requires complete buffer
	if len(m) < 5 {
		panic(fmt.Sprintf("private getter From called on invalid buffer: len(m)=%d, need at least 5 bytes", len(m)))
	}
	offsetToPrivate := int(binary.LittleEndian.Uint32(m[1:5]))
	if offsetToPrivate >= len(m) || m[offsetToPrivate] != 0x01 {
		marker := byte(0)
		...
		panic(fmt.Sprintf("private getter From called on public-only buffer: offsetToPrivate=%d, len(m)=%d, marker=0x%02x (expected 0x01)", offsetToPrivate, len(m), marker))
	}
	// Field 1 (From): nested message
	if len(m) < offsetToPrivate+1+4 {
		return nil
...
func (m CurrencyConversionRequestRaw) GetToCode() string {
	// ASSERT: Private field requires complete buffer
	if len(m) < 5 {
		panic(fmt.Sprintf("private getter ToCode called on invalid buffer: len(m)=%d, need at least 5 bytes", len(m)))
	}
	offsetToPrivate := int(binary.LittleEndian.Uint32(m[1:5]))
	if offsetToPrivate >= len(m) || m[offsetToPrivate] != 0x01 {
		marker := byte(0)
		...
		panic(fmt.Sprintf("private getter ToCode called on public-only buffer: offsetToPrivate=%d, len(m)=%d, marker=0x%02x (expected 0x01)", offsetToPrivate, len(m), marker))
	}
	// Field 2 (ToCode): variable-length
	if len(m) < offsetToPrivate+5+4 {
		return ""
...
func (m *CurrencyConversionRequestRaw) SetFrom(v MoneyRaw) error {
	// ASSERT: Private field setter requires complete buffer
	if len(*m) < 5 {
		panic(fmt.Sprintf("private setter From called on invalid buffer: len(m)=%d, need at least 5 bytes", len(*m)))
	}
	offsetToPrivate := int(binary.LittleEndian.Uint32((*m)[1:5]))
	if offsetToPrivate >= len(*m) || (*m)[offsetToPrivate] != 0x01 {
		marker := byte(0)
		...
		panic(fmt.Sprintf("private setter From called on public-only buffer: offsetToPrivate=%d, len(m)=%d, marker=0x%02x (expected 0x01)", offsetToPrivate, len(*m), marker))
	}
	// Field 1 (From): nested message
	if len(*m) < offsetToPrivate+1+4 {
		return fmt.Errorf("buffer too short for table entry")
...
func (m *CurrencyConversionRequestRaw) SetToCode(v string) error {
	// ASSERT: Private field setter requires complete buffer
	if len(*m) < 5 {
		panic(fmt.Sprintf("private setter ToCode called on invalid buffer: len(m)=%d, need at least 5 bytes", len(*m)))
	}
	offsetToPrivate := int(binary.LittleEndian.Uint32((*m)[1:5]))
	if offsetToPrivate >= len(*m) || (*m)[offsetToPrivate] != 0x01 {
		marker := byte(0)
		...
		panic(fmt.Sprintf("private setter ToCode called on public-only buffer: offsetToPrivate=%d, len(m)=%d, marker=0x%02x (expected 0x01)", offsetToPrivate, len(*m), marker))
	}
	// Field 2 (ToCode): variable-length
	if len(*m) < offsetToPrivate+5+4 {
		return fmt.Errorf("buffer too short for table entry")
...
func (m CreditCardInfoRaw) GetCreditCardNumber() string {
	// ASSERT: Private field requires complete buffer
	if len(m) < 5 {
		panic(fmt.Sprintf("private getter CreditCardNumber called on invalid buffer: len(m)=%d, need at least 5 bytes", len(m)))
	}
	offsetToPrivate := int(binary.LittleEndian.Uint32(m[1:5]))
	if offsetToPrivate >= len(m) || m[offsetToPrivate] != 0x01 {
		marker := byte(0)
		...
		panic(fmt.Sprintf("private getter CreditCardNumber called on public-only buffer: offsetToPrivate=%d, len(m)=%d, marker=0x%02x (expected 0x01)", offsetToPrivate, len(m), marker))
	}
	// Field 1 (CreditCardNumber): variable-length
	if len(m) < offsetToPrivate+1+4 {
		return ""
...
func (m CreditCardInfoRaw) GetCreditCardCvv() int32 {
	// ASSERT: Private field requires complete buffer
	if len(m) < 5 {
		panic(fmt.Sprintf("private getter CreditCardCvv called on invalid buffer: len(m)=%d, need at least 5 bytes", len(m)))
	}
	offsetToPrivate := int(binary.LittleEndian.Uint32(m[1:5]))
	if offsetToPrivate >= len(m) || m[offsetToPrivate] != 0x01 {
		marker := byte(0)
		...
		panic(fmt.Sprintf("private getter CreditCardCvv called on public-only buffer: offsetToPrivate=%d, len(m)=%d, marker=0x%02x (expected 0x01)", offsetToPrivate, len(m), marker))
	}
	// Field 2 (CreditCardCvv): fixed-length (4 bytes)
	if len(m) < offsetToPrivate+5+4 {
		return 0
...
func (m CreditCardInfoRaw) GetCreditCardExpirationYear() int32 {
	// ASSERT: Private field requires complete buffer
	if len(m) < 5 {
		panic(fmt.Sprintf("private getter CreditCardExpirationYear called on invalid buffer: len(m)=%d, need at least 5 bytes", len(m)))
	}
	offsetToPrivate := int(binary.LittleEndian.Uint32(m[1:5]))
	if offsetToPrivate >= len(m) || m[offsetToPrivate] != 0x01 {
		marker := byte(0)
		...
		panic(fmt.Sprintf("private getter CreditCardExpirationYear called on public-only buffer: offsetToPrivate=%d, len(m)=%d, marker=0x%02x (expected 0x01)", offsetToPrivate, len(m), marker))
	}
	// Field 3 (CreditCardExpirationYear): fixed-length (4 bytes)
	if len(m) < offsetToPrivate+9+4 {
		return 0
...
func (m CreditCardInfoRaw) GetCreditCardExpirationMonth() int32 {
	// ASSERT: Private field requires complete buffer
	if len(m) < 5 {
		panic(fmt.Sprintf("private getter CreditCardExpirationMonth called on invalid buffer: len(m)=%d, need at least 5 bytes", len(m)))
	}
	offsetToPrivate := int(binary.LittleEndian.Uint32(m[1:5]))
	if offsetToPrivate >= len(m) || m[offsetToPrivate] != 0x01 {
		marker := byte(0)
		...
		panic(fmt.Sprintf("private getter CreditCardExpirationMonth called on public-only buffer: offsetToPrivate=%d, len(m)=%d, marker=0x%02x (expected 0x01)", offsetToPrivate, len(m), marker))
	}
	// Field 4 (CreditCardExpirationMonth): fixed-length (4 bytes)
	if len(m) < offsetToPrivate+13+4 {
		return 0
...
func (m *CreditCardInfoRaw) SetCreditCardNumber(v string) error {
	// ASSERT: Private field setter requires complete buffer
	if len(*m) < 5 {
		panic(fmt.Sprintf("private setter CreditCardNumber called on invalid buffer: len(m)=%d, need at least 5 bytes", len(*m)))
	}
	offsetToPrivate := int(binary.LittleEndian.Uint32((*m)[1:5]))
	if offsetToPrivate >= len(*m) || (*m)[offsetToPrivate] != 0x01 {
		marker := byte(0)
		...
		panic(fmt.Sprintf("private setter CreditCardNumber called on public-only buffer: offsetToPrivate=%d, len(m)=%d, marker=0x%02x (expected 0x01)", offsetToPrivate, len(*m), marker))
	}
	// Field 1 (CreditCardNumber): variable-length
	if len(*m) < offsetToPrivate+1+4 {
		return fmt.Errorf("buffer too short for table entry")
...
func (m *CreditCardInfoRaw) SetCreditCardCvv(v int32) error {
	// ASSERT: Private field setter requires complete buffer
	if len(*m) < 5 {
		panic(fmt.Sprintf("private setter CreditCardCvv called on invalid buffer: len(m)=%d, need at least 5 bytes", len(*m)))
	}
	offsetToPrivate := int(binary.LittleEndian.Uint32((*m)[1:5]))
	if offsetToPrivate >= len(*m) || (*m)[offsetToPrivate] != 0x01 {
		marker := byte(0)
		...
		panic(fmt.Sprintf("private setter CreditCardCvv called on public-only buffer: offsetToPrivate=%d, len(m)=%d, marker=0x%02x (expected 0x01)", offsetToPrivate, len(*m), marker))
	}
	// Field 2 (CreditCardCvv): fixed-length (4 bytes)
	if len(*m) < offsetToPrivate+5+4 {
		return fmt.Errorf("buffer too short")
...
func (m *CreditCardInfoRaw) SetCreditCardExpirationYear(v int32) error {
	// ASSERT: Private field setter requires complete buffer
	if len(*m) < 5 {
		panic(fmt.Sprintf("private setter CreditCardExpirationYear called on invalid buffer: len(m)=%d, need at least 5 bytes", len(*m)))
	}
	offsetToPrivate := int(binary.LittleEndian.Uint32((*m)[1:5]))
	if offsetToPrivate >= len(*m) || (*m)[offsetToPrivate] != 0x01 {
		marker := byte(0)
		...
		panic(fmt.Sprintf("private setter CreditCardExpirationYear called on public-only buffer: offsetToPrivate=%d, len(m)=%d, marker=0x%02x (expected 0x01)", offsetToPrivate, len(*m), marker))
	}
	// Field 3 (CreditCardExpirationYear): fixed-length (4 bytes)
	if len(*m) < offsetToPrivate+9+4 {
		return fmt.Errorf("buffer too short")
...
func (m *CreditCardInfoRaw) SetCreditCardExpirationMonth(v int32) error {
	// ASSERT: Private field setter requires complete buffer
	if len(*m) < 5 {
		panic(fmt.Sprintf("private setter CreditCardExpirationMonth called on invalid buffer: len(m)=%d, need at least 5 bytes", len(*m)))
	}
	offsetToPrivate := int(binary.LittleEndian.Uint32((*m)[1:5]))
	if offsetToPrivate >= len(*m) || (*m)[offsetToPrivate] != 0x01 {
		marker := byte(0)
		...
		panic(fmt.Sprintf("private setter CreditCardExpirationMonth called on public-only buffer: offsetToPrivate=%d, len(m)=%d, marker=0x%02x (expected 0x01)", offsetToPrivate, len(*m), marker))
	}
	// Field 4 (CreditCardExpirationMonth): fixed-length (4 bytes)
	if len(*m) < offsetToPrivate+13+4 {
		return fmt.Errorf("buffer too short")
...
func (m ChargeRequestRaw) GetAmount() MoneyRaw {
	// ASSERT: Private field requires complete buffer
	if len(m) < 5 {
		panic(fmt.Sprintf("private getter Amount called on invalid buffer: len(m)=%d, need at least 5 bytes", len(m)))
	}
	offsetToPrivate := int(binary.LittleEndian.Uint32(m[1:5]))
	if offsetToPrivate >= len(m) || m[offsetToPrivate] != 0x01 {
		marker := byte(0)
		...
		panic(fmt.Sprintf("private getter Amount called on public-only buffer: offsetToPrivate=%d, len(m)=%d, marker=0x%02x (expected 0x01)", offsetToPrivate, len(m), marker))
	}
	// Field 1 (Amount): nested message
	if len(m) < offsetToPrivate+1+4 {
		return nil
...
func (m ChargeRequestRaw) GetCreditCard() CreditCardInfoRaw {
	// ASSERT: Private field requires complete buffer
	if len(m) < 5 {
		panic(fmt.Sprintf("private getter CreditCard called on invalid buffer: len(m)=%d, need at least 5 bytes", len(m)))
	}
	offsetToPrivate := int(binary.LittleEndian.Uint32(m[1:5]))
	if offsetToPrivate >= len(m) || m[offsetToPrivate] != 0x01 {
		marker := byte(0)
		...
		panic(fmt.Sprintf("private getter CreditCard called on public-only buffer: offsetToPrivate=%d, len(m)=%d, marker=0x%02x (expected 0x01)", offsetToPrivate, len(m), marker))
	}
	// Field 2 (CreditCard): nested message
	if len(m) < offsetToPrivate+5+4 {
		return nil
...
func (m *ChargeRequestRaw) SetAmount(v MoneyRaw) error {
	// ASSERT: Private field setter requires complete buffer
	if len(*m) < 5 {
		panic(fmt.Sprintf("private setter Amount called on invalid buffer: len(m)=%d, need at least 5 bytes", len(*m)))
	}
	offsetToPrivate := int(binary.LittleEndian.Uint32((*m)[1:5]))
	if offsetToPrivate >= len(*m) || (*m)[offsetToPrivate] != 0x01 {
		marker := byte(0)
		...
		panic(fmt.Sprintf("private setter Amount called on public-only buffer: offsetToPrivate=%d, len(m)=%d, marker=0x%02x (expected 0x01)", offsetToPrivate, len(*m), marker))
	}
	// Field 1 (Amount): nested message
	if len(*m) < offsetToPrivate+1+4 {
		return fmt.Errorf("buffer too short for table entry")
...
func (m *ChargeRequestRaw) SetCreditCard(v CreditCardInfoRaw) error {
	// ASSERT: Private field setter requires complete buffer
	if len(*m) < 5 {
		panic(fmt.Sprintf("private setter CreditCard called on invalid buffer: len(m)=%d, need at least 5 bytes", len(*m)))
	}
	offsetToPrivate := int(binary.LittleEndian.Uint32((*m)[1:5]))
	if offsetToPrivate >= len(*m) || (*m)[offsetToPrivate] != 0x01 {
		marker := byte(0)
		...
		panic(fmt.Sprintf("private setter CreditCard called on public-only buffer: offsetToPrivate=%d, len(m)=%d, marker=0x%02x (expected 0x01)", offsetToPrivate, len(*m), marker))
	}
	// Field 2 (CreditCard): nested message
	if len(*m) < offsetToPrivate+5+4 {
		return fmt.Errorf("buffer too short for table entry")
...
func (m ChargeResponseRaw) GetTransactionId() string {
	// ASSERT: Private field requires complete buffer
	if len(m) < 5 {
		panic(fmt.Sprintf("private getter TransactionId called on invalid buffer: len(m)=%d, need at least 5 bytes", len(m)))
	}
	offsetToPrivate := int(binary.LittleEndian.Uint32(m[1:5]))
	if offsetToPrivate >= len(m) || m[offsetToPrivate] != 0x01 {
		marker := byte(0)
		...
		panic(fmt.Sprintf("private getter TransactionId called on public-only buffer: offsetToPrivate=%d, len(m)=%d, marker=0x%02x (expected 0x01)", offsetToPrivate, len(m), marker))
	}
	// Field 1 (TransactionId): variable-length
	if len(m) < offsetToPrivate+1+4 {
		return ""
...
func (m *ChargeResponseRaw) SetTransactionId(v string) error {
	// ASSERT: Private field setter requires complete buffer
	if len(*m) < 5 {
		panic(fmt.Sprintf("private setter TransactionId called on invalid buffer: len(m)=%d, need at least 5 bytes", len(*m)))
	}
	offsetToPrivate := int(binary.LittleEndian.Uint32((*m)[1:5]))
	if offsetToPrivate >= len(*m) || (*m)[offsetToPrivate] != 0x01 {
		marker := byte(0)
		...
		panic(fmt.Sprintf("private setter TransactionId called on public-only buffer: offsetToPrivate=%d, len(m)=%d, marker=0x%02x (expected 0x01)", offsetToPrivate, len(*m), marker))
	}
	// Field 1 (TransactionId): variable-length
	if len(*m) < offsetToPrivate+1+4 {
		return fmt.Errorf("buffer too short for table entry")
...
func (m OrderItemRaw) GetCost() MoneyRaw {
	// ASSERT: Private field requires complete buffer
	if len(m) < 5 {
		panic(fmt.Sprintf("private getter Cost called on invalid buffer: len(m)=%d, need at least 5 bytes", len(m)))
	}
	offsetToPrivate := int(binary.LittleEndian.Uint32(m[1:5]))
	if offsetToPrivate >= len(m) || m[offsetToPrivate] != 0x01 {
		marker := byte(0)
		...
		panic(fmt.Sprintf("private getter Cost called on public-only buffer: offsetToPrivate=%d, len(m)=%d, marker=0x%02x (expected 0x01)", offsetToPrivate, len(m), marker))
	}
	// Field 2 (Cost): nested message
	if len(m) < offsetToPrivate+5+4 {
		return nil
...
func (m *OrderItemRaw) SetCost(v MoneyRaw) error {
	// ASSERT: Private field setter requires complete buffer
	if len(*m) < 5 {
		panic(fmt.Sprintf("private setter Cost called on invalid buffer: len(m)=%d, need at least 5 bytes", len(*m)))
	}
	offsetToPrivate := int(binary.LittleEndian.Uint32((*m)[1:5]))
	if offsetToPrivate >= len(*m) || (*m)[offsetToPrivate] != 0x01 {
		marker := byte(0)
		...
		panic(fmt.Sprintf("private setter Cost called on public-only buffer: offsetToPrivate=%d, len(m)=%d, marker=0x%02x (expected 0x01)", offsetToPrivate, len(*m), marker))
	}
	// Field 2 (Cost): nested message
	if len(*m) < offsetToPrivate+5+4 {
		return fmt.Errorf("buffer too short for table entry")
...
func (m OrderResultRaw) GetOrderId() string {
	// ASSERT: Private field requires complete buffer
	if len(m) < 5 {
		panic(fmt.Sprintf("private getter OrderId called on invalid buffer: len(m)=%d, need at least 5 bytes", len(m)))
	}
	offsetToPrivate := int(binary.LittleEndian.Uint32(m[1:5]))
	if offsetToPrivate >= len(m) || m[offsetToPrivate] != 0x01 {
		marker := byte(0)
		...
		panic(fmt.Sprintf("private getter OrderId called on public-only buffer: offsetToPrivate=%d, len(m)=%d, marker=0x%02x (expected 0x01)", offsetToPrivate, len(m), marker))
	}
	// Field 1 (OrderId): variable-length
	if len(m) < offsetToPrivate+1+4 {
		return ""
...
func (m OrderResultRaw) GetShippingTrackingId() string {
	// ASSERT: Private field requires complete buffer
	if len(m) < 5 {
		panic(fmt.Sprintf("private getter ShippingTrackingId called on invalid buffer: len(m)=%d, need at least 5 bytes", len(m)))
	}
	offsetToPrivate := int(binary.LittleEndian.Uint32(m[1:5]))
	if offsetToPrivate >= len(m) || m[offsetToPrivate] != 0x01 {
		marker := byte(0)
		...
		panic(fmt.Sprintf("private getter ShippingTrackingId called on public-only buffer: offsetToPrivate=%d, len(m)=%d, marker=0x%02x (expected 0x01)", offsetToPrivate, len(m), marker))
	}
	// Field 2 (ShippingTrackingId): variable-length
	if len(m) < offsetToPrivate+5+4 {
		return ""
...
func (m OrderResultRaw) GetShippingCost() MoneyRaw {
	// ASSERT: Private field requires complete buffer
	if len(m) < 5 {
		panic(fmt.Sprintf("private getter ShippingCost called on invalid buffer: len(m)=%d, need at least 5 bytes", len(m)))
	}
	offsetToPrivate := int(binary.LittleEndian.Uint32(m[1:5]))
	if offsetToPrivate >= len(m) || m[offsetToPrivate] != 0x01 {
		marker := byte(0)
		...
		panic(fmt.Sprintf("private getter ShippingCost called on public-only buffer: offsetToPrivate=%d, len(m)=%d, marker=0x%02x (expected 0x01)", offsetToPrivate, len(m), marker))
	}
	// Field 3 (ShippingCost): nested message
	if len(m) < offsetToPrivate+9+4 {
		return nil
...
func (m OrderResultRaw) GetShippingAddress() AddressRaw {
	// ASSERT: Private field requires complete buffer
	if len(m) < 5 {
		panic(fmt.Sprintf("private getter ShippingAddress called on invalid buffer: len(m)=%d, need at least 5 bytes", len(m)))
	}
	offsetToPrivate := int(binary.LittleEndian.Uint32(m[1:5]))
	if offsetToPrivate >= len(m) || m[offsetToPrivate] != 0x01 {
		marker := byte(0)
		...
		panic(fmt.Sprintf("private getter ShippingAddress called on public-only buffer: offsetToPrivate=%d, len(m)=%d, marker=0x%02x (expected 0x01)", offsetToPrivate, len(m), marker))
	}
	// Field 4 (ShippingAddress): nested message
	if len(m) < offsetToPrivate+13+4 {
		return nil
...
func (m *OrderResultRaw) SetOrderId(v string) error {
	// ASSERT: Private field setter requires complete buffer
	if len(*m) < 5 {
		panic(fmt.Sprintf("private setter OrderId called on invalid buffer: len(m)=%d, need at least 5 bytes", len(*m)))
	}
	offsetToPrivate := int(binary.LittleEndian.Uint32((*m)[1:5]))
	if offsetToPrivate >= len(*m) || (*m)[offsetToPrivate] != 0x01 {
		marker := byte(0)
		...
		panic(fmt.Sprintf("private setter OrderId called on public-only buffer: offsetToPrivate=%d, len(m)=%d, marker=0x%02x (expected 0x01)", offsetToPrivate, len(*m), marker))
	}
	// Field 1 (OrderId): variable-length
	if len(*m) < offsetToPrivate+1+4 {
		return fmt.Errorf("buffer too short for table entry")
...
func (m *OrderResultRaw) SetShippingTrackingId(v string) error {
	// ASSERT: Private field setter requires complete buffer
	if len(*m) < 5 {
		panic(fmt.Sprintf("private setter ShippingTrackingId called on invalid buffer: len(m)=%d, need at least 5 bytes", len(*m)))
	}
	offsetToPrivate := int(binary.LittleEndian.Uint32((*m)[1:5]))
	if offsetToPrivate >= len(*m) || (*m)[offsetToPrivate] != 0x01 {
		marker := byte(0)
		...
		panic(fmt.Sprintf("private setter ShippingTrackingId called on public-only buffer: offsetToPrivate=%d, len(m)=%d, marker=0x%02x (expected 0x01)", offsetToPrivate, len(*m), marker))
	}
	// Field 2 (ShippingTrackingId): variable-length
	if len(*m) < offsetToPrivate+5+4 {
		return fmt.Errorf("buffer too short for table entry")
...
func (m *OrderResultRaw) SetShippingCost(v MoneyRaw) error {
	// ASSERT: Private field setter requires complete buffer
	if len(*m) < 5 {
		panic(fmt.Sprintf("private setter ShippingCost called on invalid buffer: len(m)=%d, need at least 5 bytes", len(*m)))
	}
	offsetToPrivate := int(binary.LittleEndian.Uint32((*m)[1:5]))
	if offsetToPrivate >= len(*m) || (*m)[offsetToPrivate] != 0x01 {
		marker := byte(0)
		...
		panic(fmt.Sprintf("private setter ShippingCost called on public-only buffer: offsetToPrivate=%d, len(m)=%d, marker=0x%02x (expected 0x01)", offsetToPrivate, len(*m), marker))
	}
	// Field 3 (ShippingCost): nested message
	if len(*m) < offsetToPrivate+9+4 {
		return fmt.Errorf("buffer too short for table entry")
...
func (m *OrderResultRaw) SetShippingAddress(v AddressRaw) error {
	// ASSERT: Private field setter requires complete buffer
	if len(*m) < 5 {
		panic(fmt.Sprintf("private setter ShippingAddress called on invalid buffer: len(m)=%d, need at least 5 bytes", len(*m)))
	}
	offsetToPrivate := int(binary.LittleEndian.Uint32((*m)[1:5]))
	if offsetToPrivate >= len(*m) || (*m)[offsetToPrivate] != 0x01 {
		marker := byte(0)
		...
		panic(fmt.Sprintf("private setter ShippingAddress called on public-only buffer: offsetToPrivate=%d, len(m)=%d, marker=0x%02x (expected 0x01)", offsetToPrivate, len(*m), marker))
	}
	// Field 4 (ShippingAddress): nested message
	if len(*m) < offsetToPrivate+13+4 {
		return fmt.Errorf("buffer too short for table entry")
...
func (m SendOrderConfirmationRequestRaw) GetEmail() string {
	// ASSERT: Private field requires complete buffer
	if len(m) < 5 {
		panic(fmt.Sprintf("private getter Email called on invalid buffer: len(m)=%d, need at least 5 bytes", len(m)))
	}
	offsetToPrivate := int(binary.LittleEndian.Uint32(m[1:5]))
	if offsetToPrivate >= len(m) || m[offsetToPrivate] != 0x01 {
		marker := byte(0)
		...
		panic(fmt.Sprintf("private getter Email called on public-only buffer: offsetToPrivate=%d, len(m)=%d, marker=0x%02x (expected 0x01)", offsetToPrivate, len(m), marker))
	}
	// Field 1 (Email): variable-length
	if len(m) < offsetToPrivate+1+4 {
		return ""
...
func (m SendOrderConfirmationRequestRaw) GetOrder() OrderResultRaw {
	// ASSERT: Private field requires complete buffer
	if len(m) < 5 {
		panic(fmt.Sprintf("private getter Order called on invalid buffer: len(m)=%d, need at least 5 bytes", len(m)))
	}
	offsetToPrivate := int(binary.LittleEndian.Uint32(m[1:5]))
	if offsetToPrivate >= len(m) || m[offsetToPrivate] != 0x01 {
		marker := byte(0)
		...
		panic(fmt.Sprintf("private getter Order called on public-only buffer: offsetToPrivate=%d, len(m)=%d, marker=0x%02x (expected 0x01)", offsetToPrivate, len(m), marker))
	}
	// Field 2 (Order): nested message
	if len(m) < offsetToPrivate+5+4 {
		return nil
...
func (m *SendOrderConfirmationRequestRaw) SetEmail(v string) error {
	// ASSERT: Private field setter requires complete buffer
	if len(*m) < 5 {
		panic(fmt.Sprintf("private setter Email called on invalid buffer: len(m)=%d, need at least 5 bytes", len(*m)))
	}
	offsetToPrivate := int(binary.LittleEndian.Uint32((*m)[1:5]))
	if offsetToPrivate >= len(*m) || (*m)[offsetToPrivate] != 0x01 {
		marker := byte(0)
		...
		panic(fmt.Sprintf("private setter Email called on public-only buffer: offsetToPrivate=%d, len(m)=%d, marker=0x%02x (expected 0x01)", offsetToPrivate, len(*m), marker))
	}
	// Field 1 (Email): variable-length
	if len(*m) < offsetToPrivate+1+4 {
		return fmt.Errorf("buffer too short for table entry")
...
func (m *SendOrderConfirmationRequestRaw) SetOrder(v OrderResultRaw) error {
	// ASSERT: Private field setter requires complete buffer
	if len(*m) < 5 {
		panic(fmt.Sprintf("private setter Order called on invalid buffer: len(m)=%d, need at least 5 bytes", len(*m)))
	}
	offsetToPrivate := int(binary.LittleEndian.Uint32((*m)[1:5]))
	if offsetToPrivate >= len(*m) || (*m)[offsetToPrivate] != 0x01 {
		marker := byte(0)
		...
		panic(fmt.Sprintf("private setter Order called on public-only buffer: offsetToPrivate=%d, len(m)=%d, marker=0x%02x (expected 0x01)", offsetToPrivate, len(*m), marker))
	}
	// Field 2 (Order): nested message
	if len(*m) < offsetToPrivate+5+4 {
		return fmt.Errorf("buffer too short for table entry")
...
func (m PlaceOrderRequestRaw) GetUserCurrency() string {
	// ASSERT: Private field requires complete buffer
	if len(m) < 5 {
		panic(fmt.Sprintf("private getter UserCurrency called on invalid buffer: len(m)=%d, need at least 5 bytes", len(m)))
	}
	offsetToPrivate := int(binary.LittleEndian.Uint32(m[1:5]))
	if offsetToPrivate >= len(m) || m[offsetToPrivate] != 0x01 {
		marker := byte(0)
		...
		panic(fmt.Sprintf("private getter UserCurrency called on public-only buffer: offsetToPrivate=%d, len(m)=%d, marker=0x%02x (expected 0x01)", offsetToPrivate, len(m), marker))
	}
	// Field 2 (UserCurrency): variable-length
	if len(m) < offsetToPrivate+5+4 {
		return ""
...
func (m *PlaceOrderRequestRaw) SetUserCurrency(v string) error {
	// ASSERT: Private field setter requires complete buffer
	if len(*m) < 5 {
		panic(fmt.Sprintf("private setter UserCurrency called on invalid buffer: len(m)=%d, need at least 5 bytes", len(*m)))
	}
	offsetToPrivate := int(binary.LittleEndian.Uint32((*m)[1:5]))
	if offsetToPrivate >= len(*m) || (*m)[offsetToPrivate] != 0x01 {
		marker := byte(0)
		...
		panic(fmt.Sprintf("private setter UserCurrency called on public-only buffer: offsetToPrivate=%d, len(m)=%d, marker=0x%02x (expected 0x01)", offsetToPrivate, len(*m), marker))
	}
	// Field 2 (UserCurrency): variable-length
	if len(*m) < offsetToPrivate+5+4 {
		return fmt.Errorf("buffer too short for table entry")
...
func (m AdRequestRaw) GetContextKeys() []string {
	// ASSERT: Private field requires complete buffer
	if len(m) < 5 {
		panic(fmt.Sprintf("private getter ContextKeys called on invalid buffer: len(m)=%d, need at least 5 bytes", len(m)))
	}
	offsetToPrivate := int(binary.LittleEndian.Uint32(m[1:5]))
	if offsetToPrivate >= len(m) || m[offsetToPrivate] != 0x01 {
		marker := byte(0)
		...
		panic(fmt.Sprintf("private getter ContextKeys called on public-only buffer: offsetToPrivate=%d, len(m)=%d, marker=0x%02x (expected 0x01)", offsetToPrivate, len(m), marker))
	}
	// Field 2 (ContextKeys): repeated variable-length
	if len(m) < offsetToPrivate+5+4 {
		return nil
...
func (m *AdRequestRaw) SetContextKeys(v []string) error {
	// ASSERT: Private field setter requires complete buffer
	if len(*m) < 5 {
		panic(fmt.Sprintf("private setter ContextKeys called on invalid buffer: len(m)=%d, need at least 5 bytes", len(*m)))
	}
	offsetToPrivate := int(binary.LittleEndian.Uint32((*m)[1:5]))
	if offsetToPrivate >= len(*m) || (*m)[offsetToPrivate] != 0x01 {
		marker := byte(0)
		...
		panic(fmt.Sprintf("private setter ContextKeys called on public-only buffer: offsetToPrivate=%d, len(m)=%d, marker=0x%02x (expected 0x01)", offsetToPrivate, len(*m), marker))
	}
	// Field 2 (ContextKeys): repeated variable-length
	if len(*m) < offsetToPrivate+5+4 {
		return fmt.Errorf("buffer too short for table entry")
...
func (m AdResponseRaw) GetAds() []AdRaw {
	// ASSERT: Private field requires complete buffer
	if len(m) < 5 {
		panic(fmt.Sprintf("private getter Ads called on invalid buffer: len(m)=%d, need at least 5 bytes", len(m)))
	}
	offsetToPrivate := int(binary.LittleEndian.Uint32(m[1:5]))
	if offsetToPrivate >= len(m) || m[offsetToPrivate] != 0x01 {
		marker := byte(0)
		...
		panic(fmt.Sprintf("private getter Ads called on public-only buffer: offsetToPrivate=%d, len(m)=%d, marker=0x%02x (expected 0x01)", offsetToPrivate, len(m), marker))
	}
	// Field 1 (Ads): repeated nested message
	if len(m) < offsetToPrivate+1+4 {
		return nil
...
func (m *AdResponseRaw) SetAds(v []AdRaw) error {
	// ASSERT: Private field setter requires complete buffer
	if len(*m) < 5 {
		panic(fmt.Sprintf("private setter Ads called on invalid buffer: len(m)=%d, need at least 5 bytes", len(*m)))
	}
	offsetToPrivate := int(binary.LittleEndian.Uint32((*m)[1:5]))
	if offsetToPrivate >= len(*m) || (*m)[offsetToPrivate] != 0x01 {
		marker := byte(0)
		...
		panic(fmt.Sprintf("private setter Ads called on public-only buffer: offsetToPrivate=%d, len(m)=%d, marker=0x%02x (expected 0x01)", offsetToPrivate, len(*m), marker))
	}
	// Field 1 (Ads): repeated nested message
	if len(*m) < offsetToPrivate+1+4 {
		return fmt.Errorf("buffer too short for table entry")
...
func (m AdRaw) GetRedirectUrl() string {
	// ASSERT: Private field requires complete buffer
	if len(m) < 5 {
		panic(fmt.Sprintf("private getter RedirectUrl called on invalid buffer: len(m)=%d, need at least 5 bytes", len(m)))
	}
	offsetToPrivate := int(binary.LittleEndian.Uint32(m[1:5]))
	if offsetToPrivate >= len(m) || m[offsetToPrivate] != 0x01 {
		marker := byte(0)
		...
		panic(fmt.Sprintf("private getter RedirectUrl called on public-only buffer: offsetToPrivate=%d, len(m)=%d, marker=0x%02x (expected 0x01)", offsetToPrivate, len(m), marker))
	}
	// Field 1 (RedirectUrl): variable-length
	if len(m) < offsetToPrivate+1+4 {
		return ""
...
func (m AdRaw) GetText() string {
	// ASSERT: Private field requires complete buffer
	if len(m) < 5 {
		panic(fmt.Sprintf("private getter Text called on invalid buffer: len(m)=%d, need at least 5 bytes", len(m)))
	}
	offsetToPrivate := int(binary.LittleEndian.Uint32(m[1:5]))
	if offsetToPrivate >= len(m) || m[offsetToPrivate] != 0x01 {
		marker := byte(0)
		...
		panic(fmt.Sprintf("private getter Text called on public-only buffer: offsetToPrivate=%d, len(m)=%d, marker=0x%02x (expected 0x01)", offsetToPrivate, len(m), marker))
	}
	// Field 2 (Text): variable-length
	if len(m) < offsetToPrivate+5+4 {
		return ""
...
func (m *AdRaw) SetRedirectUrl(v string) error {
	// ASSERT: Private field setter requires complete buffer
	if len(*m) < 5 {
		panic(fmt.Sprintf("private setter RedirectUrl called on invalid buffer: len(m)=%d, need at least 5 bytes", len(*m)))
	}
	offsetToPrivate := int(binary.LittleEndian.Uint32((*m)[1:5]))
	if offsetToPrivate >= len(*m) || (*m)[offsetToPrivate] != 0x01 {
		marker := byte(0)
		...
		panic(fmt.Sprintf("private setter RedirectUrl called on public-only buffer: offsetToPrivate=%d, len(m)=%d, marker=0x%02x (expected 0x01)", offsetToPrivate, len(*m), marker))
	}
	// Field 1 (RedirectUrl): variable-length
	if len(*m) < offsetToPrivate+1+4 {
		return fmt.Errorf("buffer too short for table entry")
...
func (m *AdRaw) SetText(v string) error {
	// ASSERT: Private field setter requires complete buffer
	if len(*m) < 5 {
		panic(fmt.Sprintf("private setter Text called on invalid buffer: len(m)=%d, need at least 5 bytes", len(*m)))
	}
	offsetToPrivate := int(binary.LittleEndian.Uint32((*m)[1:5]))
	if offsetToPrivate >= len(*m) || (*m)[offsetToPrivate] != 0x01 {
		marker := byte(0)
		...
		panic(fmt.Sprintf("private setter Text called on public-only buffer: offsetToPrivate=%d, len(m)=%d, marker=0x%02x (expected 0x01)", offsetToPrivate, len(*m), marker))
	}
	// Field 2 (Text): variable-length
	if len(*m) < offsetToPrivate+5+4 {
		return fmt.Errorf("buffer too short for table entry")
