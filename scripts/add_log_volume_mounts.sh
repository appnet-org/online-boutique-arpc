...
# appear in /users/aruj/online-boutique-arpc/logs on the host machine
...
for dir in $DIRS; do
  for service in $SERVICES; do
    file="/users/aruj/online-boutique-arpc/$dir/$service.yaml"
    if [ -f "$file" ]; then
...
echo "Logs will be written to: /users/aruj/online-boutique-arpc/logs"
